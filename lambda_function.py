# code adapted from sample 'lambda_function.py' file created by lomibaod
# original code available at https://github.com/lomibaod/lexV2Demos/tree/main/LexV2LambdaDemoPython
...
# handles the store hours intent
def process_store_hours(intent_request):
    session_attributes = get_session_attributes(intent_request)
    slots = get_slots(intent_request)
    message = {
        'contentType': 'PlainText',
        'content': 'Service Hours: Mon-Fri 7am-7pm Sat 8am-5pm'
    }
    fulfillment_state = "Fulfilled"
    return close(intent_request, session_attributes, fulfillment_state, message)
...
def dispatch(intent_request):
    try:
        ...
        if intent_name == 'Hello':
            ...
            return process_store_hours(intent_request)
